...
describe("PendingQueueView", () => {
  ...
  it("renders empty state when no imports", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/No pending receipts to review/i)).toBeInTheDocument();
    });
  });
  ...
  it("renders imports when data is loaded", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText("1 receipt awaiting approval")).toBeInTheDocument();
    });
  });

  it("handles error during loading", async () => {
    const consoleSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    vi.mocked(tauri.getPendingImports).mockRejectedValue(new Error("Failed to fetch"));
